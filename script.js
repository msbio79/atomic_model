...
function setupControls() {
    ...
    canvas.addEventListener('touchmove', (e) => {
        if (e.touches.length === 2 && initialPinchDistance) {
            ...
            zoom = Math.min(Math.max(initialZoom * scale, 0.4), 5.0);
            updateZoomText();
        }
    }, { passive: false });
...
function updateUI() {
    ...
    const orbitalElem = document.getElementById('orbitalConfig');
    if (orbitalElem) {
        orbitalElem.innerHTML = getOrbitalConfigHTML(currentZ);
    }
...
// 오비탈 전자 배치 문자열 생성 (껍질(n) 순서대로 정렬 및 예외 처리 포함)
function getOrbitalConfigHTML(z) {
    if (z === 0) return '오비탈 배열 없음';
    let e = z;
    ...
    let terms = [];
    for (let i = 0; i < order.length; i++) {
        const capacity = order[i].cap;
        const fill = Math.min(e, capacity);
        if (fill > 0) {
            terms.push({ n: order[i].n, l: order[i].l, count: fill });
        }
        e -= fill;
        if (e <= 0) break;
    }
    
    // 대표적인 특수 오비탈 배열 (안정화) 예외 처리
    if (z === 24) { terms.find(t=>t.n===4 && t.l==='s').count = 1; terms.find(t=>t.n===3 && t.l==='d').count = 5; } // Cr
    if (z === 29) { terms.find(t=>t.n===4 && t.l==='s').count = 1; terms.find(t=>t.n===3 && t.l==='d').count = 10; } // Cu
    if (z === 42) { terms.find(t=>t.n===5 && t.l==='s').count = 1; terms.find(t=>t.n===4 && t.l==='d').count = 5; } // Mo
    if (z === 47) { terms.find(t=>t.n===5 && t.l==='s').count = 1; terms.find(t=>t.n===4 && t.l==='d').count = 10; } // Ag
    if (z === 79) { terms.find(t=>t.n===6 && t.l==='s').count = 1; terms.find(t=>t.n===5 && t.l==='d').count = 10; } // Au

    const l_values = {'s': 0, 'p': 1, 'd': 2, 'f': 3};
    terms.sort((a, b) => {
        if (a.n !== b.n) return a.n - b.n;
        return l_values[a.l] - l_values[b.l];
    });

    return terms.map(t => `${t.n}${t.l}<sup>${t.count}</sup>`).join(' ');
}
